...
import com.nswebkit.core.iface.NSSyncExposedJsApi;
...
/**
 * 自定义Cordova控件 1、可用于Activity、Fragment集成 2、可在布局xml文件中引入 3、可在代码中使用new关键字创建实例
 *
 * <p>
 * 使用示例： String launchUrl = "file:///android_asset/cordovaTest.html"; CordovaView cordovaView =
 * view.findViewById(R.id.cv); cordovaView.initCordova(getActivity());
 * cordovaView.loadUrl(launchUrl);
 * <p>
 * <p>
 * 作者：Neil 日期：2023/5/11
 */
public class NSCordovaView extends RelativeLayout implements Serializable {
   ...
   * @param context 上下文
   */
  public NSCordovaView(Context context) {
    super(context);
  }
   ...
   * @param attrs   属性
   */
  public NSCordovaView(Context context, AttributeSet attrs) {
    super(context, attrs);
  }
   ...
   * @param activity 页面
   */
  public void initCordova(Activity activity, NSPluginInterface pluginInterface) {
  ...
  /**
   * 加载配置信息 1.读取默认启动的url  file:///android_asset/www/index.html 2.读取res/xml/config.xml文件，获得插件集合
   * pluginEntries
   */
  ...
  private void initCordova() {
    ...
    //添加js同步调用native接口
    NSSyncExposedJsApi syncExposedJsApi = new NSSyncExposedJsApi(
        new NSJsBridge(appView.getPluginManager()));
    getWebview().addJavascriptInterface(syncExposedJsApi, "NSExecSync");
  ...
  private CordovaInterfaceImpl makeCordovaInterface() {
    return new CordovaInterfaceImpl(activity) {
      ...
      public Object onMessage(String id, Object data) {
        return NSCordovaView.this.onMessage(id, data);
      }
    };
  }
  ...
  public NSPluginInterface getPluginInterface() {
