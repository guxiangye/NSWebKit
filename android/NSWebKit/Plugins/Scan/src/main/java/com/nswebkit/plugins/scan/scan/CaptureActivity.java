...
package com.nswebkit.plugins.scan.scan;
...
import com.nswebkit.plugins.scan.scan.other.MNCustomViewBindCallback;
import com.nswebkit.plugins.scan.scan.other.OnScanCallback;
import com.nswebkit.plugins.scan.scan.utils.ImageUtils;
import com.nswebkit.plugins.scan.scan.utils.StatusBarUtil;
import com.nswebkit.plugins.scan.scan.utils.ZXingUtils;
import com.nswebkit.plugins.scan.scan.view.ProgressDialog;
import com.nswebkit.plugins.scan.scan.view.ScanActionMenuView;
...
public class CaptureActivity extends Activity {
    ...
    private void initView() {
        ...
        scanSurfaceView.setOnScanCallback(new OnScanCallback() {
            @Override
            public void onScanSuccess(final Result[] rawResult, Bitmap barcode) {
                UIHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        finishSuccess(rawResult[0].getText(),rawResult[0].getBarcodeFormat().toString());
                    }
                }, 200);
            }
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (TextUtils.isEmpty(decodeQRCodeFromBitmap)) {
                                ...
                                finishSuccess(decodeQRCodeFromBitmap,null);
                            }
                        }
                    });
                }
    ...
    private void finishSuccess(String result,String type) {
        Intent intent = new Intent();
        intent.putExtra(MNScanManager.INTENT_KEY_RESULT_SUCCESS, result);
        intent.putExtra(MNScanManager.INTENT_KEY_RESULT_TYPE, type);
