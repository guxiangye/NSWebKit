...
- (void)pushDatabaseBackInPool:(NSFMDatabase*)db;
- (NSFMDatabase*)db;

@end


@implementation NSFMDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return NSFMDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return NSFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return NSFMDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"NSFMdb.%@", self] UTF8String], NULL);
        _databaseInPool     = NSFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = NSFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    NSFMDBRelease(_path);
    NSFMDBRelease(_databaseInPool);
    NSFMDBRelease(_databaseOutPool);
    NSFMDBRelease(_vfsName);
    
    if (_lockQueue) {
        NSFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(NSFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The NSFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
