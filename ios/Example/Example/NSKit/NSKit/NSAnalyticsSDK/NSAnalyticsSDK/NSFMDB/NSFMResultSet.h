...
/** Represents the results of executing a query on an `<NSFMDatabase>`.
...
@interface NSFMResultSet : NSObject

@property (nonatomic, retain, nullable) NSFMDatabase *parentDB;
...
/** `NSFMStatement` used by result set. */

@property (atomic, retain, nullable) NSFMStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<NSFMStatement>`
 
 @param statement A `<NSFMStatement>` to be performed
 
 @param aDB A `<NSFMDatabase>` to be used
 
 @return A `NSFMResultSet` on success; `nil` on failure
 */
