...
- (NSFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args;
...
#pragma mark NSFMDatabase instantiation and deallocation

+ (instancetype)databaseWithPath:(NSString *)aPath {
    return NSFMDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}

+ (instancetype)databaseWithURL:(NSURL *)url {
    return NSFMDBReturnAutoreleased([[self alloc] initWithURL:url]);
}
...
- (void)dealloc {
    [self close];
    NSFMDBRelease(_openResultSets);
    NSFMDBRelease(_cachedStatements);
    NSFMDBRelease(_dateFormat);
    NSFMDBRelease(_databasePath);
    NSFMDBRelease(_openFunctions);
...
+ (NSString*)NSFMDBUserVersion {
    return @"2.7.5";
}

// returns 0x0240 for version 2.4.  This makes it super easy to do things like:
// /* need to make sure to do X with NSFMDB version 2.4 or later */
// if ([NSFMDatabase NSFMDBVersion] >= 0x0240) { … }

+ (SInt32)NSFMDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 NSFMDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self NSFMDBUserVersion];
        ...
        char *e = nil;
        NSFMDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);
        
    });
    
    return NSFMDBVersionVal;
}
...
static int NSFMDBDatabaseBusyHandler(void *f, int count) {
    NSFMDatabase *self = (__bridge NSFMDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &NSFMDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to NSFMDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"NSFMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"NSFMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = NSFMDBReturnAutoreleased([_openResultSets copy]);
...
- (void)resultSetDidClose:(NSFMResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (NSFMStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (NSFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(NSFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(NSFMStatement*)statement forQuery:(NSString*)query {
    NSParameterAssert(query);
    if (!query) {
        NSLog(@"API misuse, -[NSFMDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = NSFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = NSFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    NSFMDBAutorelease(_dateFormat);
    _dateFormat = NSFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    NSFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The NSFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The NSFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The NSFMDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The NSFMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"NSFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (NSFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (NSFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    NSFMStatement *statement  = 0x00;
    NSFMResultSet *rs         = 0x00;
    ...
    NSFMDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[NSFMStatement alloc] init];
    ...
    rs = [NSFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (NSFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (NSFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (NSFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (NSFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    NSFMResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (NSFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[NSFMStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        NSFMDBRelease(cachedStmt);
    }
...
int NSFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int NSFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) NSFMDBExecuteStatementsCallbackBlock)block {
    
    int rc;
    char *errmsg = nil;
    
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? NSFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *NSFMDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", NSFMDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", NSFMDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", NSFMDBEscapeSavePointName(name)];
...
- (BOOL)checkpoint:(NSFMDBCheckpointMode)checkpointMode error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:nil logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(NSFMDBCheckpointMode)checkpointMode name:(NSString *)name error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:name logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(NSFMDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError * __autoreleasing *)error
{
...
void NSFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void NSFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString *)name arguments:(int)arguments block:(void (^)(void *context, int argc, void **argv))block {
    ...
    id b = NSFMDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &NSFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void*)b, &NSFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
