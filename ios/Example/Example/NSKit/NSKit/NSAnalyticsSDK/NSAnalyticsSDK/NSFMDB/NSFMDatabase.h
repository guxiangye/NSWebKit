...
#import "NSFMResultSet.h"
#import "NSFMDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define NSFMDBAutorelease(__v) ([__v autorelease]);
    #define NSFMDBReturnAutoreleased NSFMDBAutorelease

    #define NSFMDBRetain(__v) ([__v retain]);
    #define NSFMDBReturnRetained NSFMDBRetain

    #define NSFMDBRelease(__v) ([__v release]);

    #define NSFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define NSFMDBAutorelease(__v)
    #define NSFMDBReturnAutoreleased(__v) (__v)

    #define NSFMDBRetain(__v)
    #define NSFMDBReturnRetained(__v) (__v)

    #define NSFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define NSFMDBDispatchQueueRelease(__v)
    #else
        #define NSFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^NSFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, NSFMDBCheckpointMode) {
    NSFMDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    NSFMDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    NSFMDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    NSFMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in NSFMDB are:

 - `NSFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<NSFMResultSet>` - Represents the results of executing a query on an `NSFMDatabase`.
 - `<NSFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<NSFMDatabasePool>` - A pool of `NSFMDatabase` objects.
 - `<NSFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [NSFMDB on GitHub](https://github.com/ccgus/NSFMdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [NSFMDB mailing list](http://groups.google.com/group/NSFMdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `NSFMDatabase` object and use it across multiple threads. Instead, use `<NSFMDatabaseQueue>`.
...
@interface NSFMDatabase : NSObject
...
/** Create a `NSFMDatabase` object.
 
 An `NSFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `NSFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `NSFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    NSFMDatabase *db = [NSFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    NSFMDatabase *db     = [NSFMDatabase databaseWithPath:dbPath];
 ...
 @return `NSFMDatabase` object if successful; `nil` if failure.
...
/** Create a `NSFMDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `NSFMDatabase` connection is closed.
...
/** Initialize a `NSFMDatabase` object.
...
/** Initialize a `NSFMDatabase` object.
 ...
 An `NSFMDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) NSFMDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<NSFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[NSFMResultSet next]>`) from one record to the other.
 ...
 @return A `<NSFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see NSFMResultSet
 @see [`NSFMResultSet next`](<[NSFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (NSFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (NSFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (NSFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> NSFMResultSet!`
...
- (NSFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (NSFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (NSFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    NSFMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [NSFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one NSFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using NSFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to NSFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using NSFMDB you will not need to interact directly with `NSFMStatement`, but rather with `<NSFMDatabase>` and `<NSFMResultSet>` only.
...
@interface NSFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
