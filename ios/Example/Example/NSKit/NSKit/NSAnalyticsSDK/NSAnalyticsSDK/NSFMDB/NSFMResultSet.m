...
@interface NSFMResultSet () {
    NSMutableDictionary *_columnNameToIndexMap;
}
@end

@implementation NSFMResultSet

+ (instancetype)resultSetWithStatement:(NSFMStatement *)statement usingParentDatabase:(NSFMDatabase*)aDB {
    
    NSFMResultSet *rs = [[NSFMResultSet alloc] init];
    ...
    return NSFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    NSFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return NSFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"NSFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
