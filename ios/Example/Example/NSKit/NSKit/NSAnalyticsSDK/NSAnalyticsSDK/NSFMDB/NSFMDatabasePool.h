...
/** Pool of `<NSFMDatabase>` objects.
 ...
 @warning Before using `NSFMDatabasePool`, please consider using `<NSFMDatabaseQueue>` instead.

 If you really really really know what you're doing and `NSFMDatabasePool` is what
 you really really need (ie, you're using a read only database), OK you can use
 it.  But just be careful not to deadlock!

 For an example on deadlocking, search for:
 `ONLY_USE_THE_POOL_IF_YOU_ARE_DOING_READS_OTHERWISE_YOULL_DEADLOCK_USE_NSFMDATABASEQUEUE_INSTEAD`
 in the main.m file.
 */

@interface NSFMDatabasePool : NSObject
...
/** Create pool using path.
 ...
 @return The `NSFMDatabasePool` object. `nil` on error.
 */
...
/** Synchronously perform database operations in pool.
 ...
 @param block The code to be run on the `NSFMDatabasePool` pool.
 */
...
/** Synchronously perform database operations in pool using save point.
 ...
 @warning You can not nest these, since calling it will pull another database out of the pool and you'll get a deadlock. If you need to nest, use `<[NSFMDatabase startSavePointWithName:error:]>` instead.
*/
...
/** NSFMDatabasePool delegate category
 
 This is a category that defines the protocol for the NSFMDatabasePool delegate
 */

@interface NSObject (NSFMDatabasePoolDelegate)

/** Asks the delegate whether database should be added to the pool. 
 
 @param pool     The `NSFMDatabasePool` object.
 @param database The `NSFMDatabase` object.
 
 @return `YES` if it should add database to pool; `NO` if not.
 
 */

- (BOOL)databasePool:(NSFMDatabasePool*)pool shouldAddDatabaseToPool:(NSFMDatabase*)database;
...
- (void)databasePool:(NSFMDatabasePool*)pool didAddDatabase:(NSFMDatabase*)database;
