...
#import "SDPFMResultSet.h"
#import "SDPFMDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define SDPFMDBAutorelease(__v) ([__v autorelease]);
    #define SDPFMDBReturnAutoreleased SDPFMDBAutorelease

    #define SDPFMDBRetain(__v) ([__v retain]);
    #define SDPFMDBReturnRetained SDPFMDBRetain

    #define SDPFMDBRelease(__v) ([__v release]);

    #define SDPFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define SDPFMDBAutorelease(__v)
    #define SDPFMDBReturnAutoreleased(__v) (__v)

    #define SDPFMDBRetain(__v)
    #define SDPFMDBReturnRetained(__v) (__v)

    #define SDPFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define SDPFMDBDispatchQueueRelease(__v)
    #else
        #define SDPFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^SDPFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, SDPFMDBCheckpointMode) {
    SDPFMDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    SDPFMDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    SDPFMDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    SDPFMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in SDPFMDB are:

 - `SDPFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<SDPFMResultSet>` - Represents the results of executing a query on an `SDPFMDatabase`.
 - `<SDPFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<SDPFMDatabasePool>` - A pool of `SDPFMDatabase` objects.
 - `<SDPFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [SDPFMDB on GitHub](https://github.com/ccgus/SDPFMdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [SDPFMDB mailing list](http://groups.google.com/group/SDPFMdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `SDPFMDatabase` object and use it across multiple threads. Instead, use `<SDPFMDatabaseQueue>`.
...
@interface SDPFMDatabase : NSObject
...
/** Create a `SDPFMDatabase` object.
 
 An `SDPFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `SDPFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `SDPFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    SDPFMDatabase *db = [SDPFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    SDPFMDatabase *db     = [SDPFMDatabase databaseWithPath:dbPath];
 ...
 @return `SDPFMDatabase` object if successful; `nil` if failure.
...
/** Create a `SDPFMDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `SDPFMDatabase` connection is closed.
...
/** Initialize a `SDPFMDatabase` object.
...
/** Initialize a `SDPFMDatabase` object.
 ...
 An `SDPFMDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) SDPFMDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<SDPFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[SDPFMResultSet next]>`) from one record to the other.
 ...
 @return A `<SDPFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see SDPFMResultSet
 @see [`SDPFMResultSet next`](<[SDPFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (SDPFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (SDPFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (SDPFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> SDPFMResultSet!`
...
- (SDPFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (SDPFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (SDPFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Begin a transaction
 ...
 @warning    Unlike SQLite's `BEGIN TRANSACTION`, this method currently performs
             ...
             is likely to change in future versions of SDPFMDB, whereby this method
 ...
 */
- (BOOL)checkpoint:(SDPFMDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(SDPFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(SDPFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
+ (NSString*)SDPFMDBUserVersion;

+ (SInt32)SDPFMDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    SDPFMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [SDPFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one SDPFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using SDPFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to SDPFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using SDPFMDB you will not need to interact directly with `SDPFMStatement`, but rather with `<SDPFMDatabase>` and `<SDPFMResultSet>` only.
...
@interface SDPFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
