...
@class SDPFMDatabase;
@class SDPFMStatement;

/** Represents the results of executing a query on an `<SDPFMDatabase>`.
...
@interface SDPFMResultSet : NSObject

@property (nonatomic, retain, nullable) SDPFMDatabase *parentDB;
...
/** `SDPFMStatement` used by result set. */

@property (atomic, retain, nullable) SDPFMStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<SDPFMStatement>`
 
 @param statement A `<SDPFMStatement>` to be performed
 
 @param aDB A `<SDPFMDatabase>` to be used
 
 @return A `SDPFMResultSet` on success; `nil` on failure
 */
