...
- (void)pushDatabaseBackInPool:(SDPFMDatabase*)db;
- (SDPFMDatabase*)db;

@end


@implementation SDPFMDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return SDPFMDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return SDPFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return SDPFMDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"SDPFMdb.%@", self] UTF8String], NULL);
        _databaseInPool     = SDPFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = SDPFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    SDPFMDBRelease(_databaseInPool);
    SDPFMDBRelease(_databaseOutPool);
    SDPFMDBRelease(_vfsName);
    
    if (_lockQueue) {
        SDPFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(SDPFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The SDPFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
