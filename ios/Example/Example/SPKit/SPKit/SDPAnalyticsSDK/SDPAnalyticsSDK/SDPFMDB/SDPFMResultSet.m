...
@interface SDPFMResultSet () {
    NSMutableDictionary *_columnNameToIndexMap;
}
@end

@implementation SDPFMResultSet

+ (instancetype)resultSetWithStatement:(SDPFMStatement *)statement usingParentDatabase:(SDPFMDatabase*)aDB {
    
    SDPFMResultSet *rs = [[SDPFMResultSet alloc] init];
    ...
    return SDPFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    SDPFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return SDPFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"SDPFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
