...
@interface SDPReportManager()<NSURLSessionDelegate>
...
@implementation SDPReportManager

#pragma mark - 创建单例
+ (SDPReportManager * _Nullable)sharedInstance{
    static dispatch_once_t onceToken;
    static SDPReportManager *sharedInstance;
    dispatch_once(&onceToken, ^{
        sharedInstance = [SDPReportManager new];
    });
...
-(void)strategyMatch:(NSUInteger)priority{
    ...
    else if ([[self networkStatus] isEqualToString:@"NG"] && [[SDPDBManager shareDataManager] queryGatherInfoNumWithPriority:priority] >= 50 && priority == 1) {
        self.isProgress1 = YES;
...
-(void)postGatherInfoWithPriority:(NSUInteger)priority withNum:(NSInteger)num withTimeInterval:(NSInteger)seconds{
    ...
    loadInfoArray = [[SDPDBManager shareDataManager] getGatherInfoWithPriority:priority withNum:num];
    ...
    NSURLSessionDataTask *sessionDataTask = [defaultSession dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if ([resultDic[@"code"] isEqualToString:@"000000"]) {
            NSLog(@"上报成功");
            if (priority != 3) {
                [[SDPDBManager shareDataManager] deleteGatherInfoWithPriority:priority withNum:loadInfoArray.count];
            }
        }
        //优先级4 成功或失败都删除
        if (priority == 3) {
        ...
        //间隔上报
        if ([[SDPDBManager shareDataManager] queryGatherInfoNumWithPriority:priority] > 0) {
...
//        // 校验域名是否一致
//        if([SDPEncryptManager verifyServerTrust:challenge.protectionSpace.serverTrust withDomain:challenge.protectionSpace.host]) {
