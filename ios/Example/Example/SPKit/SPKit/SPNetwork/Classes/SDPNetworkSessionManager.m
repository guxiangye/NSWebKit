...
#import "SDPIPDirect.h"
#pragma mark -
NSString * SDPPercentEscapedStringFromString(NSString *string)
{
...
@interface SDPQueryStringPair : NSObject
...
@implementation SDPQueryStringPair
...
- (NSString *)URLEncodedStringValue {
    if (!self.value || [self.value isEqual:[NSNull null]]) {
        return SDPPercentEscapedStringFromString([self.field description]);
    } else {
        return [NSString stringWithFormat:@"%@=%@", SDPPercentEscapedStringFromString([self.field description]), SDPPercentEscapedStringFromString([self.value description])];
    }
}

@end
FOUNDATION_EXPORT NSArray * SDPQueryStringPairsFromDictionary(NSDictionary *dictionary);
FOUNDATION_EXPORT NSArray * SDPQueryStringPairsFromKeyAndValue(NSString *key, id value);
NSString * SDPQueryStringFromParameters(NSDictionary *parameters)
{
    NSMutableArray *mutablePairs = [NSMutableArray array];
    for (SDPQueryStringPair *pair in SDPQueryStringPairsFromDictionary(parameters)) {
...
NSArray * SDPQueryStringPairsFromDictionary(NSDictionary *dictionary)
{
    return SDPQueryStringPairsFromKeyAndValue(nil, dictionary);
}

NSArray * SDPQueryStringPairsFromKeyAndValue(NSString *key, id value)
{
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
        ...
        for (id nestedKey in [dictionary.allKeys sortedArrayUsingDescriptors:@[ sortDescriptor ]]) {
            ...
            if (nestedValue) {
                [mutableQueryStringComponents addObjectsFromArray:SDPQueryStringPairsFromKeyAndValue((key ? [NSString stringWithFormat:@"%@[%@]", key, nestedKey] : nestedKey), nestedValue)];
            }
        }
    } else if ([value isKindOfClass:[NSArray class]]) {
        NSArray *array = value;
        for (id nestedValue in array) {
            [mutableQueryStringComponents addObjectsFromArray:SDPQueryStringPairsFromKeyAndValue([NSString stringWithFormat:@"%@[]", key], nestedValue)];
        }
    } else if ([value isKindOfClass:[NSSet class]]) {
        NSSet *set = value;
        for (id obj in [set sortedArrayUsingDescriptors:@[ sortDescriptor ]]) {
            [mutableQueryStringComponents addObjectsFromArray:SDPQueryStringPairsFromKeyAndValue(key, obj)];
        }
    } else {
        [mutableQueryStringComponents addObject:[[SDPQueryStringPair alloc] initWithField:key value:value]];
    }

    return mutableQueryStringComponents;
}

@interface SDPNetworkSessionManager ()<NSURLSessionDelegate, NSURLSessionTaskDelegate, NSURLSessionDataDelegate>
...
@implementation SDPNetworkSessionManager
...
- (NSURLRequest *)handelWithRequestOptions:(SDPNetworkRequestOptions *)options {
    NSMutableURLRequest *request = nil;
    NSURL *url = [NSURL URLWithString:options.path relativeToURL:self.baseURL];
    if (options.request == nil) {
        __block SDPNetworkRequestOptions *op = options;
        [self.interceptors enumerateObjectsUsingBlock:^(SDPNetworkInterceptor *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            op = [obj onRequest:op];
        }];
        ...
        if ([options.header[@"Content-Type"] containsString:@"application/json"]) {
            ...
            NSString *bodyStr = SDPQueryStringFromParameters(op.parameters);
        ...
        if (options.useIPDirect) {
            ...
            NSString *ip = [SDPIPDirect getIPWithHostName:host];
            if (ip.length > 0) {
...
- (id)handelWithResponseOptions:(SDPNetworkResponseOptions *)options {
    __block SDPNetworkResponseOptions *op = options;
    [self.interceptors enumerateObjectsUsingBlock:^(SDPNetworkInterceptor *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (NSURLSessionDataTask *)dataTaskWithNetworkRequestOptions:(SDPNetworkRequestOptions *)requestOptions success:(void (^)(NSURLSessionDataTask *, id))success failure:(void (^)(NSURLSessionDataTask *, NSError *))failure {
    ...
    __block NSURLSessionDataTask *dataTask = [self.session dataTaskWithRequest:request completionHandler:^(NSData *_Nullable data, NSURLResponse *_Nullable response, NSError *_Nullable error) {
        SDPNetworkResponseOptions *responseOptions = [SDPNetworkResponseOptions new];
...
- (void)   URLSession:(NSURLSession *)session didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge
    ...
    if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        if ([SDPSecurityPolicy evaluateServerTrust:challenge.protectionSpace.serverTrust forDomain:domain]) {
